...
minetest.debug("[ds_sailboat] Mod loading")
...
function ds_sailboat.on_rightclick(self, clicker)

	local name = clicker:get_player_name()
	
	minetest.debug("[ds_sailboat] "..name.." & "..dump(default.player_attached[name]))
...
function ds_sailboat.on_activate(self, staticdata, dtime_s)
...
function ds_sailboat.get_staticdata()
	return tostring(v)
end

function ds_sailboat.on_punch(self, puncher, time_from_last_punch, tool_capabilities, direction)

	 -- if the ds_sailboat occupied?
	 if self.driver then
	...
	-- remove the ds_sailboat
	else

		ds_sailboat.schedule_removal(self)
		if not minetest.setting_getbool("creative_mode") then
			puncher:get_inventory():add_item("main", "ds_sailboat:ds_sailboat")
		end
	
	end
end

function ds_sailboat.on_step(self, dtime)
	...
	if self.stepcount>9 then
		...
		if self.driver then
			...
			-- remove unattended ds_sailboats after 20 counts
...
function ds_sailboat.schedule_removal(self)
...
minetest.register_entity("ds_sailboat:ds_sailboat", ds_sailboat)

minetest.register_craftitem("ds_sailboat:ds_sailboat", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		minetest.add_entity(pointed_thing.under, "ds_sailboat:ds_sailboat")
		if not minetest.setting_getbool("creative_mode") then
			itemstack:take_item()
		end
		return itemstack
...
minetest.debug("[ds_sailboat] Mod loaded")